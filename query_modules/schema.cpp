...
std::string Schema::TypeOf(const mgp::Type &type) {
  switch (type) {
    ...
    // (the PDF p37 table claim of "Integer → SQL_BIGINT" is incorrect). The driver only recognises
    // "Int" and "Long"; "Int" is kept to preserve BIGINT mapping in BI tools.
    case mgp::Type::Int:
      return "Int";
    ...
    case mgp::Type::Duration:
      return "Duration";
    // Memgraph docs call this "ZonedDateTime"; we emit "DateTime" to match what the Simba JDBC
    // connector and Neo4j APOC recognise (the SQL type table maps "DateTime" → SQL_TIMESTAMP).
    // Without this rename BI tools degrade timezone-aware timestamp columns to VARCHAR.
