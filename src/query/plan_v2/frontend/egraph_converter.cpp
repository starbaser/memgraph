...
auto BuildPreExtractionData(EGraph const &core) -> PreExtractionData {
  ...
  for (auto eclass_id : core.canonical_eclass_ids()) {
    for (auto enode_id : core.eclass(eclass_id).nodes()) {
      ...
      if (sym == symbol::Symbol) {
      ...
      } else if (sym == symbol::Identifier && !enode.children().empty()) {
        ...
        identifier_referenced.push_back(enode.children()[0]);
      }
    }
  }
...
void CheckSymbolSingletonInvariant([[maybe_unused]] EGraph const &core) {
  ...
  for (auto eclass_id : core.canonical_eclass_ids()) {
    ...
    bool const has_symbol = std::ranges::any_of(
        cls.nodes(), [&](auto enode_id) { return core.get_enode(enode_id).symbol() == symbol::Symbol; });
...
auto BuildOperatorTree(auto const &impl, QueryPlannerContext::Impl &ctx) -> BuildOutput {
  ...
  auto build_state = BuildState{
      .literal_info = impl.graph.template storage<symbol::Literal>().info,
      .named_output_info = impl.graph.template storage<symbol::NamedOutput>().info,
...
auto ConvertToLogicalOperator(egraph const &e, eclass root, QueryPlannerContext &planner_context) -> ExtractionResult {
  ...
  // Root alt: cheapest self-contained (the one the resolver would pick under
  // in_scope = {}). Existence guaranteed by the root_satisfiable check above.
  auto self_contained =
