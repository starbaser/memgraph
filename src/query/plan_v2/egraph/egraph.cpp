...
auto egraph::MakeOnce() -> eclass { return from_core(pimpl_->graph.Make<symbol::Once>()); }

auto egraph::MakeSymbol(int32_t position, std::string_view name) -> eclass {
  return from_core(pimpl_->graph.Make<symbol::Symbol>(position, name));
}

auto egraph::MakeLiteral(storage::ExternalPropertyValue const &value) -> eclass {
  return from_core(pimpl_->graph.Make<symbol::Literal>(value));
}

auto egraph::MakeParameterLookup(int32_t position) -> eclass {
  return from_core(pimpl_->graph.Make<symbol::ParamLookup>(position));
}

auto egraph::MakeBind(eclass input, eclass sym, eclass expr) -> eclass {
  return from_core(pimpl_->graph.Make<symbol::Bind>(to_core(input), to_core(sym), to_core(expr)));
}

auto egraph::MakeIdentifier(eclass sym) -> eclass {
  return from_core(pimpl_->graph.Make<symbol::Identifier>(to_core(sym)));
}

auto egraph::MakeOutput(eclass input, std::span<eclass const> named_outputs) -> eclass {
  auto children = utils::small_vector<pcore::EClassId>{};
  ...
  return from_core(pimpl_->graph.Make<symbol::Output>(std::move(children)));
}

auto egraph::MakeNamedOutput(std::string_view name, eclass sym, eclass expr) -> eclass {
  return from_core(pimpl_->graph.Make<symbol::NamedOutput>(name, to_core(sym), to_core(expr)));
}

auto egraph::MakeFunction(std::string_view name, std::span<eclass const> args) -> eclass {
  return from_core(pimpl_->graph.Make<symbol::Function>(name, to_core(args)));
}

auto egraph::MakeUnwind(eclass input, eclass sym, eclass list_expr) -> eclass {
  return from_core(pimpl_->graph.Make<symbol::Unwind>(to_core(input), to_core(sym), to_core(list_expr)));
}
...
auto egraph::MakeSubquery(eclass outer_input, eclass inner_root, std::span<eclass const> exposed_syms) -> eclass {
  ...
  return from_core(pimpl_->graph.Make<symbol::Subquery>(std::move(children)));
}

auto egraph::FunctionInfoById(std::uint64_t id) const -> FunctionInfo const * {
  auto const &info = pimpl_->graph.storage<symbol::Function>().info;
...
#define MG_DEFN_MAKE_BINARY(Name, ...)                                              \
  auto egraph::Make##Name(eclass lhs, eclass rhs)->eclass {                         \
    return from_core(pimpl_->graph.Make<symbol::Name>(to_core(lhs), to_core(rhs))); \
  }
...
#define MG_DEFN_MAKE_UNARY(Name, ...)                                     \
  auto egraph::Make##Name(eclass operand)->eclass {                       \
    return from_core(pimpl_->graph.Make<symbol::Name>(to_core(operand))); \
  }
