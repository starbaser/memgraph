...
struct egraph::impl {
  pcore::TypedEGraph<symbol, analysis, AllSymbols, symbol_make_traits> graph;
};
...
/// friendship needed.
inline auto to_core(eclass id) -> pcore::EClassId { return pcore::EClassId{id.value_of()}; }

inline auto from_core(pcore::EClassId id) -> eclass { return eclass{id.value_of()}; }
...
inline auto to_core(std::span<eclass const> ids) -> utils::small_vector<pcore::EClassId> {
  auto out = utils::small_vector<pcore::EClassId>{};
  out.reserve(ids.size());
  for (auto id : ids) out.push_back(to_core(id));
  return out;
}
