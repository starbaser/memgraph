...
// ============================================================================
// symbol_cost_traits<S> - per-symbol cost emission, dispatched on
// enode.symbol() by `DispatchBySymbol` in `CostCtx::operator()`.  Paired with
// `symbol_resolve_traits<S>` across the construction / resolution duality.
//
// Helpers and cost-class constants stay file-local in this TU (see above).
// Trait specs for class-bucketed symbols (arithmetic / comparison / boolean /
// unary) are macro-generated from the EGRAPH_*_OPS X-lists; specs for the
// structural and scope-threading symbols are hand-written.
//
// Primary template intentionally undefined - a missing specialisation surfaces
// as a compile error at the dispatcher.
// ============================================================================
...
template <>
struct symbol_cost_traits<symbol::Once> {
...
template <>
struct symbol_cost_traits<symbol::Literal> {
...
template <>
struct symbol_cost_traits<symbol::Symbol> {
  static auto cost(ENodeT const &, ENodeId id, CostChildren, CostCtx const &) -> CostFrontier {
...
template <>
struct symbol_cost_traits<symbol::ParamLookup> {
  static auto cost(ENodeT const &, ENodeId id, CostChildren, CostCtx const &) -> CostFrontier {
...
template <>
struct symbol_cost_traits<symbol::Identifier> {
...
template <>
struct symbol_cost_traits<symbol::Bind> {
...
template <>
struct symbol_cost_traits<symbol::Unwind> {
...
template <>
struct symbol_cost_traits<symbol::Output> {
...
template <>
struct symbol_cost_traits<symbol::NamedOutput> {
  ...
  static auto cost(ENodeT const &, ENodeId id, CostChildren children, CostCtx const &) -> CostFrontier {
    return BinaryCombine(*children[0], *children[1], 1.0, id);
  }
};

template <>
struct symbol_cost_traits<symbol::Function> {
...
#define MG_BINARY_COST_TRAIT(Name, ClassConst)                                                             \
  template <>                                                                                              \
  struct symbol_cost_traits<symbol::Name> {                                                                \
    static auto cost(ENodeT const &, ENodeId id, CostChildren children, CostCtx const &) -> CostFrontier { \
      return BinaryCombine(*children[0], *children[1], ClassConst, id);                                    \
    }                                                                                                      \
  };
...
#define MG_UNARY_COST_TRAIT(Name, ...)                                                                     \
  ...
  struct symbol_cost_traits<symbol::Name> {                                                                \
    static auto cost(ENodeT const &, ENodeId id, CostChildren children, CostCtx const &) -> CostFrontier { \
      return Restamp(*children[0], kUnary, id);                                                            \
    }                                                                                                      \
  };
